      ...
      <div className="page-body privacy-policy-body">
        ...
        <div className="privacy-section">
          ...
          <div className="privacy-card">
            ...
            <ul>
              <li>{isZH
                ? '对您发布的所有内容承担全部责任'
                : 'Take full responsibility for all content you post or share'}</li>
              <li>{isZH
                ? '不发布、传播或分享任何不当、违法或冒犯性内容'
                : 'Not post, transmit, or share any inappropriate, illegal, or offensive content'}</li>
              <li>{isZH
                ? '尊重其他用户，不进行任何形式的骚扰或欺凌行为'
                : 'Treat other users with respect and refrain from any form of harassment or bullying'}</li>
              <li>{isZH
                ? '不利用本应用进行任何非法活动'
                : 'Not use the App for any illegal activities'}</li>
              <li>{isZH
                ? '积极举报您发现的任何不当内容或滥用行为'
                : 'Actively report any objectionable content or abusive behavior you encounter'}</li>
              <li>{isZH
                ? '保护您的账号安全，不与他人共享登录凭据'
                : 'Keep your account secure and not share your login credentials'}</li>
            </ul>
          </div>
        </div>
        ...
        <div className="privacy-section">
          ...
          <div className="privacy-card">
            ...
            <ul>
              <li>{isZH
                ? '内容过滤：我们部署了自动化系统来检测和过滤不当内容'
                : 'Content filtering: We deploy automated systems to detect and filter objectionable content'}</li>
              <li>{isZH
                ? '用户举报：用户可以通过应用内的举报功能标记不当内容或滥用行为'
                : 'User reporting: Users can flag objectionable content or abusive behavior through the in-app reporting feature'}</li>
              <li>{isZH
                ? '用户拉黑：用户可以拉黑滥用行为的用户，拉黑后该用户的内容将立即从您的动态中移除'
                : 'User blocking: Users can block abusive users. Once blocked, the user\'s content will be instantly removed from your feed'}</li>
              <li>{isZH
                ? '人工审核：我们的内容审核团队会在收到举报后24小时内审查所有举报，并采取适当行动'
                : 'Manual review: Our content moderation team reviews all reports within 24 hours and takes appropriate action'}</li>
            </ul>
          </div>
        </div>
        ...
        <div className="privacy-section">
          ...
          <div className="privacy-card">
            <p>{isZH
              ? '我们为用户提供以下安全工具：'
              : 'We provide the following safety tools to our users:'}</p>
            <ul>
              <li>{isZH
                ? '举报内容：您可以举报任何您认为违反本条款的帖子、评论或消息。所有举报将在24小时内审查处理。'
                : 'Report content: You can report any post, comment, or message that you believe violates these terms. All reports will be reviewed and acted upon within 24 hours.'}</li>
              <li>{isZH
                ? '拉黑用户：您可以拉黑任何您认为有滥用行为的用户。拉黑操作将立即生效：该用户将无法向您发送消息，其内容将从您的动态中移除，同时开发团队将收到通知以审查该用户的行为。'
                : 'Block users: You can block any user you believe is behaving abusively. Blocking takes effect immediately: the user will be unable to send you messages, their content will be removed from your feed, and the development team will be notified to review the user\'s behavior.'}</li>
            </ul>
          </div>
        </div>
        ...
        <div className="privacy-section">
          ...
          <div className="privacy-card">
            ...
            <ul>
              <li>{isZH
                ? '立即删除违规内容'
                : 'Immediate removal of the offending content'}</li>
              <li>{isZH
                ? '对发布违规内容的用户发出警告或立即封禁账号'
                : 'Warning or immediate suspension of the account that posted the offending content'}</li>
              <li>{isZH
                ? '严重或反复违规将导致永久封禁，不予申诉'
                : 'Severe or repeated violations will result in permanent ban without appeal'}</li>
              <li>{isZH
                ? '如涉及违法行为，我们将配合执法机构进行调查'
                : 'If illegal activity is involved, we will cooperate with law enforcement agencies'}</li>
            </ul>
          </div>
        </div>
        ...
        <div className="privacy-section">
          ...
          <div className="privacy-card highlight">
            <p>{isZH
              ? '我们承诺在收到不当内容举报后24小时内采取行动。这包括：'
              : 'We commit to acting on all reports of objectionable content within 24 hours. This includes:'}</p>
            <ul>
        ...
        <div className="privacy-section">
          ...
          <div className="privacy-card">
            <p>{isZH
              ? '我们保留在以下情况下暂停或终止您的账号的权利，且无需事先通知：'
              : 'We reserve the right to suspend or terminate your account without prior notice if:'}</p>
            <ul>
        ...
        <div className="privacy-section">
          ...
          <div className="privacy-card">
            <p>{isZH
              ? '我们可能会不时更新本使用条款。更新后的条款将在应用内公布。继续使用本应用即表示您接受更新后的条款。'
              : 'We may update these Terms of Use from time to time. Updated terms will be published within the App. Your continued use of the App constitutes acceptance of the updated terms.'}</p>
          </div>
        </div>
        ...
        <div className="privacy-section">
          ...
          <div className="privacy-card">
            <p>{isZH
              ? '如果您对本使用条款有任何疑问，或需要举报不当内容或滥用行为，请通过以下方式联系我们：'
              : 'If you have any questions about these Terms of Use, or need to report objectionable content or abusive behavior, please contact us:'}</p>
            <ul>
