            ...
            <div key={m.id} className={`moment-card${imgCount === 0 && (!m.videos || m.videos.length === 0) ? ' text-only' : ''}`}>
              <div className="moment-header">
                ...
                {!isOwner && (
                  <button
                    className="icon-btn"
                    onClick={() => { setReportingId(reportingId === m.id ? null : m.id); setReportReason('') }}
                    style={{ fontSize: 14, color: 'var(--text-muted)' }}
              ...
              {reportingId === m.id && (
                <div style={{
                  ...
                  <button
                    ...
                    onClick={() => handleReportMoment(m.id)}
                    disabled={!reportReason || reportSubmitting}
                    style={{
                      background: '#ef4444', color: '#fff', border: 'none',
                      borderRadius: 6, padding: '8px 0', fontWeight: 600, fontSize: 12,
