    ...
    if (data.startsWith('paperphone://friend/')) {
      ...
      if (inviteId) {
        try {
          ...
          const groupId = res.group_id || res.id || res.groupId
          if (groupId) {
            navigate(`/group/${groupId}`)
          } else {
            console.warn('[QR] Group join response missing group_id:', res)
            setScanResult(t('discover.join_success'))
            setTimeout(() => setScanResult(''), 3000)
          }
        } catch (err) {
          console.error('[QR] Group join failed:', err)
