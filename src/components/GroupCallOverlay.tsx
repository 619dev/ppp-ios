  ...
  return (
    <main style={shellStyle}>
      <header style={headerStyle}>
        ...
        <button aria-label="参会者" onClick={() => setShowPeople(v => !v)} style={{ ...iconButton, marginLeft: 'auto' }}><Users size={20} /></button>
      </header>

      <section style={{ flex: 1, minHeight: 0, display: 'flex' }}>
        <div style={{ flex: 1, minWidth: 0, padding: 8 }}><VideoStage gc={gc} participants={participants} /></div>
...
function VideoStage({ gc, participants }: { gc: ReturnType<typeof useGroupCallContext>; participants: any[] }) {
  ...
  return (
    <div style={{ height: '100%', display: 'grid', gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`, gridAutoRows: 'minmax(0, 1fr)', gap: 6, overflow: 'hidden' }}>
...
function ParticipantTile({ participant: p }: { participant: any }) {
  ...
  return (
    <article style={{ position: 'relative', minHeight: 0, overflow: 'hidden', borderRadius: 10, background: '#151a22', display: 'flex', alignItems: 'center', justifyContent: 'center', border: p.isSpeaking ? '2px solid #38bdf8' : '2px solid transparent' }}>
