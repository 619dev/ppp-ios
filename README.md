...
- 新增消息引用回复：可引用文字及媒体消息，发送后显示原消息摘要，并支持点击引用快速定位。
- 私聊语音和视频通话全面迁移至 LiveKit SFU，改善连接建立、弱网重连和跨网络通话稳定性。
- 会话列表与通知预览可正确解析带引用信息的消息，不再显示内部消息载荷。
- 设置页新增当前应用版本显示。
- 引用回复界面完整支持中、英、日、韩、法、德、俄、西 8 种语言。
