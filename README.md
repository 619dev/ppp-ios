...
- 修复私聊语音通话无声：为远端音轨增加独立播放器，并避免视频通话中的音频重复播放。
- 修复实时变声：仅在启用变声模式时创建音频处理链，切回正常模式时恢复原始麦克风音轨。
- 优化私聊视频通话界面：通话接通后以紧凑信息卡展示联系人和状态，减少对远端画面的遮挡。
- 重新设计聊天输入区域：文本框可随内容自动增高，优化按钮尺寸、附件面板布局及中文输入法下的发送行为。
- 修复未读消息数字异常：WebSocket 重连重放离线消息时不再重复累计未读数，从推送、通知或深链直接进入聊天时会正确清零对应会话。
- 修复私聊视频通话画中画：视频元素挂载或重建后会自动重新绑定已建立的本地及远端媒体流。
