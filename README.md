...
- 新增长效设备会话与访问令牌自动刷新，短期令牌过期后可无感续期。
- WebSocket 增加认证超时、心跳检测、指数退避重连，并在网络、代理或前后台状态变化后主动恢复连接。
- 新增消息补拉、断线发送队列与客户端消息 ID 去重，重连后自动补齐遗漏消息并重发待发送内容。
- 升级旧版登录会话，只有服务端明确撤销会话时才退出，离线和网络切换不再清除本地账户。
